...
# packages <- c("readxl", "skimr", "dplyr", "ggplot2", "GGally", 
#               "corrplot", "scales", "DescTools", "reshape2", 
#               "factoextra", "readr", "kableExtra", "rrcov", 
#               "psych")
...
Alz_mortality_data_complete_with_state_csv <- read_csv("data/Alz_mortality_data_complete_with_state_csv.csv")
```
...
```{r}
USHealth=Alz_mortality_data_complete_with_state_csv
```

```{r}
unique_states <- unique(USHealth$State)
...
```{r}
filtered_USHealth <- Alz_mortality_data_complete_with_state_csv %>%
  filter(State %in% c("FL", "GA" )) %>%
  select(County, State, obesity_age_adj, Smoking_Rate,
         physical_inactivity, Diabetes, Heart_Disease, Cancer, NATA_Cancer_11, Poverty_Percent)
```

We filtered the dataset to analyze the variables obesity, smoking, physical inactivity, diabetes, heart disease, cancer, Nata Cancer, and poverty percentage in the southern states of Florida and Georgia.

```{r}
sapply(filtered_USHealth, class)
```

```{r}
sapply(filtered_USHealth, function(x) length(unique(x)))
```

**Standardize the data**
```{r}
filtered_data <- filtered_USHealth %>%
  select(-County, -State)
