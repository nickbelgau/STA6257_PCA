...
Transforming the dataset using the Box-Cox transformation resulted in an improvement by retaining more information. This is apparent by the higher explained variance.
```{r}
...
importance_original <- t(importance_original)
colnames(importance_original) <- c("Std Dev", "Proportion", "Cumulative Var")
...
ggplot(combined_results, aes(x=PC, y=`Cumulative Var`, color=name, group=name)) +
  geom_line(size=1.1) +
...
View the eigenvalues - loadings components:  
```{r}
