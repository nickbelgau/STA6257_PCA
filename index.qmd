...
## Application 2: Image Compression & Classification  
Don't use PCA for image classification. Use neural networks.[need sources]  
Don't use PCa for image compression, use JPEG or other
...
py_install("ann_visualizer", pip = TRUE)
```
...
PCA requires a 2-dimensional input (samples x features). Flatten the images from 32x32 pixels with 3 color channels (n x 32x32x3 array) into a 2-D vector (n x 3072).  
```{python}
...
PCA was applied to retain 95% of the variance, reducing dimensionality significantly while maintaining image information. The explained variance plot showed the number of components required to reach this threshold.
...
Why SVM used for image classification, traditional ML, about SVM
SVM models were created using the flattened data
kernel='rbf', C=1.0, gamma='scale'

Load the models and predictions on test data  
```{python}
import pickle
...
The training results demonstrate that the CNN model, which incorporates convolutional layers, max pooling, and dropout, effectively reduces overfitting and enhances performance over time. The architecture supports a solid learning process, as shown by the consistent reduction in validation loss and steady improvement in accuracy. The higher training loss compared to validation loss can be attributed to the 'dropout' regularization technique.
...
Metrics were compared on the test data
...
# Append the new column for model sizes
...
Cnns are faster:
CNNs are specifically designed for image data and leverage convolutional layers that apply filters to capture spatial hierarchies in images. CNNs are generally more complex in terms of architecture but are highly optimized for parallel processing. This makes them relatively faster at processing large volumes of image data once trained.

SVMs can be computationally intensive, especially if the kernel trick is used to handle non-linear classification. 
The complexity in SVMs arises from the calculations needed to determine the support vectors during training and the decision function during prediction.

Flattening leads to high computational costs.


CNNs are superior for image classification due to their ability to learn high-level features directly from data, adapt to complex patterns, and perform efficiently in real-time applications, including on IoT devices. 

machine vision

TABLE OF RESULTS

EXPLAIN HOW CNN MUCH BETTER THAN PCA FOR IMAGE CLASSIFICATION
 but CNNs are the gold standard for most current image classification tasks.
