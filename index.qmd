...
The Harvey-Collier Test for Linearity assesses linearity by fitting a linear model to each variable pair, calculating residuals, and conducting regression again on these residuals. A significant nonzero slope in the residual regression indicates non-linearity, suggesting potential information loss in PCA.
...
```
The test fails because the p-value is less than the significance level, so the relationship is nonlinear.  
...
The residual plot for these variables demonstrates their nonlinear relationship, explaining the test failure. Although transformations can adjust linearity, they risk altering other variable relationships. Thus, no transformations were applied, acknowledging some information loss in PCA.
...
The Box-Cox transformation is a robust statistical method that normalizes the data distribution, enhancing its suitability for PCA. It is considered best practice to apply such transformations to columns selectively rather than on the entirety of the dataset. Automating this process can be achieved by setting a kurtosis threshold.Additionally, negative values must be carefully managed to ensure the Box-Cox transformation is correctly implemented. 
```{r}
library(MASS)
