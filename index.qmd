...
The Harvey-Collier Test for Linearity fits a linear model to each variable pair, calculates residuals, and conducts regression again on these residuals. A significant nonzero slope in the residual regression indicates non-linearity, suggesting potential information loss in PCA.
...
ggplot(data_residual, aes(x = obesity_age_adj, y = residuals)) +
  geom_point() + 
  geom_smooth(method = "loess", se = FALSE, color = "blue") +  # Add the LOWESS curve
...
library(ggplot2)
# Adding an identifier column to each data frame
importance_transform$Data <- "Transformed"
importance$Data <- "Original"

# Combining both data frames for plotting
combined_importance <- rbind(importance_transform, importance)

# Convert row names to a column for plotting
combined_importance$PC <- rownames(combined_importance)

# # Create line plot
# ggplot(combined_importance, aes(x = PC, y = `Cumulative Var`, color = Data, group = Data)) +
#   geom_line() +
